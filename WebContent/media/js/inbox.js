var Inbox = function () {
    ...
    var loadInbox = function (name,url,searchname) {
        ...
        $.getJSON(url,data, function (res) {
        	...
        	chtml+='<li><a href="javascript:void(0)" id="del-btn"><i class="icon-trash"></i>删除</a></li>'; 
        	chtml+='</ul>'; 
        	chtml+='</div> </th>'; 
        	...
        	for ( var int = 0; int < res.fs.length; int++) {
        		...
        		chtml+='<tr class="unread">'+
        	     '<td class="inbox-small-cells"> <input type="checkbox" class="mail-checkbox checkbox-del" value="'+row.id+'" /> </td>';
    ...
    return {
        ...
        init: function () {
            ...
            $("#del-btn").live('click',function(){
            	var ids = "";
            	$(".checkbox-del").each(function(i,e){
            		if($(e).prop('checked')){
            			ids += $(e).val() + ',';
            			$(e).closest('tr').remove();
            		}
            	});
            	var url = 'mail_flow!del?ids='+ids;
        		$.get(url,function(d){
        			if (d=="success") {
        				alert('删除成功');
        			}
            ...
            if (App.getURLParameter("a") === "view") {
            	...
            	$.ajax({
    				...
    				success:function(d){
    					if(d.trim()!=""){
    		            	$('.inbox-nav > li.inbox > a').text('新消息('+d+')');
    					}
    				},
    				error:function(){
    					alert("网络异常!");
